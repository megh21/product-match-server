...
from tensorrt_llm.quantization import quantize_and_export, quantize_nemo_and_export

if __name__ == "__main__":
    mp.set_start_method("spawn", force=True)
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--model_dir", help="Specify where the HuggingFace model is", default=None
    )
    parser.add_argument(
        "--nemo_ckpt_path", help="Specify where the NeMo checkpoint is", default=None
    )
    parser.add_argument(
        "--decoder_type",
        type=str,
    ...
    if args.autoq_format:
        ...
        if (
            ...
            or args.auto_quantize_bits < lower_bound
            or args.auto_quantize_bits > upper_bound
        ):
            print(f"invalid auto_quantize_bits value, will be set to {lower_bound}")
    ...
    if args.model_dir is not None:
        ...
        quantize_nemo_and_export(
            nemo_ckpt_path=args.nemo_ckpt_path,
